...
int forward_from_backend_minimal_level_04b(int argc, char* argv[]) {

    lvl04b::Backend backend{};
    lvl04b::Foo foo{};
    foo.val = 0;
    
    lvl04b::im_param::parameter(backend, "foo", foo);
    ASSERT(foo.val == 1);
    ...
    lvl04b::im_param::parameter(backend, "x", x);
    ASSERT(x == 2);
